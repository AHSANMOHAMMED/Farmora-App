        ...
        data: (prices) => ListView(
          ...
          children: [
            Container(
              ...
              child: Row(
                children: [
                  const Icon(Icons.info_outline, color: Color(0xff1f7a4d)),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Text(
                      'Daily prices from Dambulla and Manning Market (${prices.length} crops)',
                      style: const TextStyle(color: Color(0xff1f7a4d)),
                    ),
                  ),
                ],
              ),
...
class _PriceChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(_getCropEmoji(price.cropName), style: const TextStyle(fontSize: 16)),
          const SizedBox(width: 6),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            mainAxisSize: MainAxisSize.min,
            children: [
              Text(price.cropName, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
              Text(price.displayPrice, style: const TextStyle(fontSize: 11, color: Color(0xff1f7a4d), fontWeight: FontWeight.w700)),
            ],
          ),
        ],
      ),
  ...
  String _getCropEmoji(String cropName) {
    const cropEmoji = {
      'Tomato': '🍅', 'Carrot': '🥕', 'Green Chilli': '🌶️',
      'Capsicum': '🫑', 'Brinjal': '🍆', 'Banana': '🍌',
      'Mango': '🥭', 'Papaya': '🍈', 'Leeks': '🧅',
