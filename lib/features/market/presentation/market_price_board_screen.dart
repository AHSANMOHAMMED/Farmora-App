...
class _MarketPriceBoardScreenState extends State<MarketPriceBoardScreen> {
  ...
  // Created once so rebuilds don't resubscribe.
  final Stream<List<MarketPriceIndex>> _prices =
      FirestoreService().marketPricesStream();
  @override
  Widget build(BuildContext context) => Scaffold(
    ...
    body: StreamBuilder<List<MarketPriceIndex>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) return Center(child: Text(userMessage(snapshot.error!, action: 'load market prices'), textAlign: TextAlign.center));
  ...
  Future<void> _report() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(userMessage(error, action: 'submit market price'))));
    }
  }
}
